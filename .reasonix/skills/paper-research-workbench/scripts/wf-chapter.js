 ...
 * --save-as  : Final download path (default: E:/Downloads/Firefox/<auto-name>)
  ...
  const dlResult = await new Promise(resolve => {
    ...
    function listen(p) {
      p.on('download', async (dl) => {
        const dest = saveAs || path.join(DOWNLOAD_DIR, dl.suggestedFilename());
    ...
    (async () => {
      ...
      const effectiveExpand = expandList.length > 0 ? expandList
        : (autoCheckCount > 0 && diag.tier === 'nested') ? diag.nodes.filter(n => n.hasArrow && /^第.+章/.test(n.title)).slice(0, autoCheckCount).map(n => n.title)
        : [];
        ...
        for (const item of items) {
          ...
          for (const ch of effectiveExpand) {
            if (txt.includes(ch) && !/\d+[.．]\d+/.test(txt)) {
              ...
              if (await arrow.count() > 0) { await arrow.click(); expanded++; await chPage.waitForTimeout(200); }
              break;
            }
          }
        }
