/**
 * ff-run.js — Run arbitrary Playwright code (generic fallback)
 * 
 * Usage:
 *   node ff-run.js (--code <js-body> | --code-file <path> | --stdin)
 *                  [--expect-download] [--save-as <path> | --download-dir <dir>]
 *                  [--no-close] [--browser <firefox|chrome|edge>] [--timeout <ms>]
 */
...
const downloadDir = opt('--download-dir', DOWNLOAD_DIR);
...
if (!code) {
  console.error('Usage: node ff-run.js (--code <js-body> | --code-file <path> | --stdin) [--expect-download] [--save-as <path>] [--no-close] [--browser <browser>]');
  process.exit(1);
}
